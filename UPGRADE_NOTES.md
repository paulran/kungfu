...
```bash
# libnode 已克隆到本地
git clone https://github.com/paulran/libnode.git framework/core/.deps/libnode

# 更新 libnode 版本配置
cd framework/core/.deps/libnode
git checkout v24.15.0
git submodule sync
```

**修改文件**: `framework/core/.deps/libnode/package.json`
- 版本更新到 `24.15.0`
...
**步骤 2**: 更新子模块配置（可选，如需从源码编译）
```bash
git submodule sync
# git submodule update --init --recursive  # 此步骤会克隆完整 node 仓库，非常耗时
```
...
3. 进入 framework/core 目录，运行 `yarn compile` 验证编译成功
